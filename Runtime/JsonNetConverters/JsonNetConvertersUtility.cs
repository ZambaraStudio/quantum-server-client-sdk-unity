...
namespace Newtonsoft.Json.UnityConverters.Utility
{
    ...
    /// </code>
    /// 
    public static class JsonNetConvertersUtility
    {
        /// <summary>
        ...
        private static Type[] FindConverterTypes()
        {
            ...
            var converterTypes = types.Where((type) => typeof(JsonConverter).IsAssignableFrom(type));
            var result = converterTypes.Where((type) => (!type.IsAbstract && !type.IsGenericTypeDefinition))
                .Where((type) => null != type.GetConstructor(new Type[0]))
                .Where((type) => (null != type.Namespace && type.Namespace.StartsWith("Newtonsoft.Json")))
                .OrderBy((type) =>
                    null != type.Namespace && type.Namespace.StartsWith("Newtonsoft.Json.UnityConverters"))
                .ToArray();
            return result;
        }
